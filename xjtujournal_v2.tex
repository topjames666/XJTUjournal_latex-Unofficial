%==============================================================================
% 西安交通大学学报 LaTeX 模板 - 严格按照开发规范文档2修改
% XJTU Journal LaTeX Template - Strictly Following Development Standards v2.0
%
% 编译器：XeLaTeX
% Menu -> Compiler -> XeLaTeX
%
% 依据文档：《开发规范文档2_格式修正版.md》
% 修正内容：页眉字号、标题对齐、标题间距、一级标题加粗、行间距和段落格式
%==============================================================================
...
\geometry{
    ...
    bottom=2.54cm,
    headheight=27pt,
    headsep=0.5cm,  % ✅ 修正：从0.7cm改为0.5cm，使页眉更紧凑
    footskip=1.2cm
}
...
% 字体定义
%------------------------------------------------------------------------------
\newcommand{\songti}{\CJKfamily{fandol}}
\newcommand{\heiti}{\CJKfamily{fandolhei}}
\newcommand{\kaishu}{\CJKfamily{fandolkai}}
...
%------------------------------------------------------------------------------
% 行距和段落格式设置（修正版：单倍行距+首行缩进）
%------------------------------------------------------------------------------
\singlespacing  % ✅ 修正：从\onehalfspacing改为\singlespacing（单倍行距）
\setlength{\parindent}{0.85cm}    % ✅ 修正：首行缩进0.85cm
\setlength{\parskip}{0pt}         % ✅ 修正：段间距0pt（段前段后都是0）
...
\fancypagestyle{firstpage}{
  \fancyhf{}
  % 左栏：中文卷期信息（5号宋体，左对齐）✅ 修正：从小5号改为5号
  \fancyhead[L]{
      \wuhao\songti  % ✅ 修正：从\xiaowuhao改为\wuhao
      第56卷第x期\\
      2022年x月
  }
  % 中栏：期刊名称（中英文，居中）
  ...
  % 右栏：英文卷期信息（5号Times New Roman，右对齐）✅ 修正：从小5号改为5号
  \fancyhead[R]{
      \wuhao\textrm  % ✅ 修正：从\xiaowuhao改为\wuhao
      Vol.56 No.x\\
      Aug. 2022
  }
  % 页脚：页码（5号宋体，居中）✅ 修正：从小5号改为5号
  \fancyfoot[C]{\wuhao\songti \thepage}  % ✅ 修正：从\xiaowuhao改为\wuhao
  % 页眉横线：必须存在（细实线）
...
%------------------ 普通页面样式：奇偶页不同 ------------------
% 奇数页（右页，CO）：页眉中间为论文标题（5号宋体）✅ 修正：从小5号改为5号
\fancyhead[CO]{\wuhao\songti \papertitle}  % ✅ 修正：从\xiaowuhao改为\wuhao

% 偶数页（左页，CE）：页眉中间为期刊名（5号宋体）✅ 修正：从小5号改为5号
\fancyhead[CE]{\wuhao\songti 西安交通大学学报}  % ✅ 修正：从\xiaowuhao改为\wuhao

% 页脚：所有非首页页面页脚均为中间页码（5号宋体）✅ 修正：从小5号改为5号
\fancyfoot[C]{\wuhao\songti \thepage}  % ✅ 修正：从\xiaowuhao改为\wuhao
...
%------------------------------------------------------------------------------
% 标题格式设置（修正版：左对齐，不加粗，设置间距）
%------------------------------------------------------------------------------
\usepackage{titlesec}

% 一级标题：四号宋体，左对齐，不加粗 ✅ 修正：从居中改为左对齐，去掉加粗
\titleformat{\section}
    {\xiaosihao\songti}  % ✅ 修正：去掉\centering和\bfseries
    {\thesection}
    {1em}
    {}

% 一级标题间距：段前0.5行，段后0行 ✅ 新增
\titlespacing*{\section}
    {0pt}                      % 左缩进：0
    {0.5\baselineskip}         % 段前：0.5行
    {0pt}                      % 段后：0行

% 二级标题：五号黑体，左对齐，加粗（保持不变）
...
% 二级标题间距：段前0.5行，段后0.5行 ✅ 新增
\titlespacing*{\subsection}
    {0pt}                      % 左缩进：0
    {0.5\baselineskip}         % 段前：0.5行
    {0.5\baselineskip}         % 段后：0.5行
...
%===【正文开始 - 双栏排版】====================================================
% 注意：段落格式已在preamble中设置（单倍行距+首行缩进0.85cm+段间距0pt）
