%==============================================================================
% 西安交通大学学报 LaTeX 模板 - 严格按照开发规范文档3修改
% XJTU Journal LaTeX Template - Strictly Following Development Standards v3.0
%
% 编译器：XeLaTeX
% Menu -> Compiler -> XeLaTeX
%
% 依据文档：《开发规范文档3_标题和页眉修正版.md》
% 修正内容：标题加粗、页眉位置、页眉内容格式
%==============================================================================
...
\geometry{
    ...
    top=2.54cm,
    bottom=2.54cm,
    headheight=30pt,  % ✅ 修正：从27pt增加到30pt，为页眉文字留出更多空间
    headsep=0.4cm,    % ✅ 修正：从0.5cm减小到0.4cm，使页眉更紧凑
    footskip=1.2cm
}
...
\fancypagestyle{firstpage}{
  ...
  % 左栏：中文卷期信息（5号宋体，左对齐）
  % ✅ 修正：使用\raisebox调整垂直位置，紧贴页眉线但不触碰
  \fancyhead[L]{
      \raisebox{-0.5pt}{  % ✅ 修正：向下微调0.5pt，紧贴但不触碰
          \wuhao\songti
          第56卷第x期\\
          2022年x月
      }
  ...
  % 中栏：期刊名称（中英文，居中）
  % ✅ 修正：中文不加粗，英文一行显示，调整垂直位置
  \fancyhead[C]{
      \raisebox{2pt}{  % ✅ 修正：向上微调2pt，不能压在线上
          \begin{minipage}{0.5\textwidth}  % ✅ 修正：增加宽度以容纳一行英文
          \centering
          \sihao\heiti      % ✅ 修正：去掉\bfseries，中文期刊名不加粗
          西安交通大学学报\\
          \sihao\textrm\bfseries     % 英文期刊名：4号Times New Roman加粗
          JOURNAL OF XI'AN JIAOTONG UNIVERSITY  % ✅ 修正：一行显示，不换行
          \end{minipage}
      }
  }
...
%------------------ 普通页面样式：奇偶页不同 ------------------
% 奇数页（右页，CO）：页眉中间为论文标题（5号宋体）
\fancyhead[CO]{\wuhao\songti \papertitle}

% 偶数页（左页，CE）：页眉中间为期刊名（5号宋体）
\fancyhead[CE]{\wuhao\songti 西安交通大学学报}

% 页脚：所有非首页页面页脚均为中间页码（5号宋体）
\fancyfoot[C]{\wuhao\songti \thepage}
...
% 一级标题：四号宋体，左对齐，不加粗 ✅ 修正：去掉\bfseries
\titleformat{\section}
    {\xiaosihao\songti}  % ✅ 修正：不加粗，只使用普通宋体
    {\thesection}
    {1em}
    {}
...
% 二级标题：五号黑体，左对齐，不加粗 ✅ 修正：去掉\bfseries
\titleformat{\subsection}
    {\wuhao\heiti}  % ✅ 修正：去掉\bfseries，只使用普通黑体
    {\thesubsection}
    {1em}
    {}

% 二级标题间距：段前0.5行，段后0.5行
